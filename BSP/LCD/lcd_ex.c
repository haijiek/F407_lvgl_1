...
/**
 * @brief       ILI9486寄存器初始化代码
 * @param       无
 * @retval      无
 */
void lcd_ex_ili9486_reginit(void)
{
    /* Step 1: 软件复位 */
    lcd_wr_regno(0x01);          // Software Reset
    HAL_Delay(150);              // 必须 > 120ms
    ...
    lcd_wr_data(0x55);           // 0x55 = 16bit (必须!)

    /* Step 4: 设置扫描方向 */
    lcd_wr_regno(0x36);          // Memory Access Control
    lcd_wr_data(0x48);           // 0x48 = 横屏, BGR模式 (根据实际调整)

    /* Step 5: 电源设置（关键！） */
    lcd_wr_regno(0xC0);          // Power Control 1
    lcd_wr_data(0x0E);           // VRH[5:0] = 14 (4.1V)
    lcd_wr_data(0x0E);           // SAP[2:0] = 2, BT[3:0] = 0

    lcd_wr_regno(0xC1);          // Power Control 2
    lcd_wr_data(0x41);           // VGH = VCI1 x 2, VGL = VCI1 x 2

    lcd_wr_regno(0xC5);          // VCOM Control
    lcd_wr_data(0x00);           // VCOMH = 2.4V
    lcd_wr_data(0x53);           // VCOML = -2.4V (关键值!)
    lcd_wr_data(0x80);           // VCOM offset

    /* Step 6: 帧率控制 */
    lcd_wr_regno(0xB1);          // Frame Rate Control
    lcd_wr_data(0x00);           // Fosc/2
    lcd_wr_data(0x1B);           // 70Hz

    /* Step 7: 显示控制 */
    lcd_wr_regno(0xB4);          // Display Inversion Control
    lcd_wr_data(0x02);           // 2-dot inversion

    lcd_wr_regno(0xB6);          // Display Function Control
    lcd_wr_data(0x02);           // REV = 0, GS = 0
    lcd_wr_data(0x02);           // SS = 0, SM = 0
    ...
    lcd_wr_data(0x00);
    lcd_wr_data(0x01);
    lcd_wr_data(0x3F);           // 0x13F = 319 (320像素宽)

    lcd_wr_regno(0x2B);          // Page Address Set
    lcd_wr_data(0x00);
    lcd_wr_data(0x00);
    lcd_wr_data(0x01);
    lcd_wr_data(0xDF);           // 0x1DF = 479 (480像素高)

    /* Step 9: Gamma 设置（重要！） */
    lcd_wr_regno(0xE0);          // Positive Gamma
    lcd_wr_data(0x0F);
    lcd_wr_data(0x31);
    lcd_wr_data(0x2B);
    lcd_wr_data(0x0C);
    ...
    lcd_wr_regno(0xE1);          // Negative Gamma
    lcd_wr_data(0x00);
    lcd_wr_data(0x0E);
    lcd_wr_data(0x14);
    lcd_wr_data(0x03);
    ...
    /* Step 10: 开显示 */
    lcd_wr_regno(0x13);          // Normal Display Mode On
    HAL_Delay(10);
